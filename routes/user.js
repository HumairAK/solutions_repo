...
var returnRouter = function(io) {
    ...
    router.get('/user_profile', loggedIn, isUser, function (req, res, next) {
        ...
        function getNotifications() {
            return new Promise(function(resolve, reject){
                dbFile.getNotifications(req.user.email, function(success, error, data, message) {
                    if (error){
                        console.log(message);
                        resolve(1);
                    }
                        ...
                        req.user.notifications = data; //data is an array
                        console.log("notification retrieved. Length: " + data.length);
                        resolve(1);
                    }
                });
            });
        ...
        getComments().then(getMail).then(getFollows).then(getNotifications).then(function (data) {
                        ...
                        if (error) {
                            ...
                            transporter.sendMail(mailOptions, function (error, info) {
                                if (error) {
                                    ...
                                    res.render('reset_password', {
                                        success: "We've e-mailed you instructions for resetting your password." +
    ...
    router.post('/submit_solution/:examID/:qID', function (req, res) {
        ...
        if (!req.body.solution) {
            ...
            dbFile.add_solution(fields, function (addedSolution, statusMsg) {
                ...
                // Will need to make another call to the database to add more info to the solution object about the
                // exam - for notifications
                var solution = {
                    examID: examID,
                    qID: qID,
                    text: text,
                ...
                io.sockets.emit('new_solution', solution);
    ...
    io.on('connection', function(socket){
        socket.on('add_notification', function(data){
            ...
            dbFile.addNotification(local_req.user.email, data, function(err, message){
                console.log(message);
            });
